...
class Admin extends \lib\core\AbstractAdmin {
	...
	private function newPost() {
		...
		// Insert
		$this->pdbc->query('INSERT IGNORE INTO `module_block` (`id_theme`, `name`, `content`)
	...
	private function editPost($id) {
		...
		// Update
		$this->pdbc->query('UPDATE IGNORE `module_block`
	...
	private function removeGet($id) {
		return array(
			'message' => '<p>Are you sure you want to remove this block? This action can\'t be undone!</p>'
		);
	}
