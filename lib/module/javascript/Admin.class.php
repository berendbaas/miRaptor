...
class Admin extends \lib\core\AbstractAdmin {
	...
	private function newPost() {
		...
		if(!isset($_POST['theme'], $_POST['name'], $_POST['javascript'])) {
			$field['error'] = '<p class="msg-warning">Require theme, name & javascript.</p>';
			return $field;
		}
		...
		try {
			...
			$field['error'] = '<p class="msg-error">This javascript already exists. Please try again.</p>';
			return $field;
		}
	...
	private function newPage($field) {
		...
		return '<h2 class="icon icon-module-javascript">Edit javascript</h2>' . $field['error'] . $form->__toString();
	}
	...
	private function removePost($id) {
		try {
			...
			return array(
				'error' => '<p class="msg-error">Can\'t remove javascript.</p>'
			);
		}
	...
	private function removePage($field) {
		...
		return '<h2 class="icon icon-module-javascript">Remove javascript</h2>' . $field['error'] . $form->__toString();
	}
}
